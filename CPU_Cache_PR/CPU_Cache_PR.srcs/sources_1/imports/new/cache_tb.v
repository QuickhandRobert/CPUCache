...
//        .access(access),
    ...
    initial begin
        ...
        // 2) Read Addr=0 (hit)
        do_access(32'd0, 1'b0, 32'd0);
        $display("Read Addr=0 again: Data_Out=%h, Hit_Miss=%b, Acc=%0d, Miss=%0d", 
                 Data_Out, Hit_Miss, total_accesses, total_misses);

        // 3) Write Addr=4
        do_access(32'd4, 1'b1, 32'hA5A5A5A5);
        $display(" Write Addr=4: Hit_Miss=%b, Acc=%0d, Miss=%0d", 
                 Hit_Miss, total_accesses, total_misses);

        // 4) Read Addr=4 (hit)
        do_access(32'd4, 1'b0, 32'd0);
        $display("Read Addr=4: Data_Out=%h, Hit_Miss=%b, Acc=%0d, Miss=%0d", 
                  Data_Out, Hit_Miss, total_accesses, total_misses);

        // 5) Read Addr=32 (new block)
        do_access(32'd32, 1'b0, 32'd0);
        $display("Read Addr=32: Data_Out=%h, Hit_Miss=%b, Acc=%0d, Miss=%0d", 
                  Data_Out, Hit_Miss, total_accesses, total_misses);
